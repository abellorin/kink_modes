...
double F1(double x, double eta_1, double beta); 
double F2(double x, double eta_2, double beta, double omega, double t); 
double Cond_I_0(double x, double A, double B);
double Cond_I_1(double x);
void makeAgr(char *Agr, int L);

int main(int argc, char **argv)
{
	double A, B, eta_1, eta_2, beta, omega, gamma;
	...
	for(i=0;i<=M;i++) {
		...
		u[1][i] = Cond_I_0(x[i], A, B);
		u[0][i] = Cond_I_1(x[i]);
		u_t[i] = u[0][i];
		U[i] = Y_Zero - u[1][i]*(float)YMax/PS;
		f1[i] = F1(x[i], eta_1, beta);
		f[i] = f1[i] + F2(x[i], eta_2, beta, omega, t);
	...
	if(File_Flag >= 1) 
		fPoi << x_cm << v_cm << u[1][P] << u_t[P] << endl;
	if(File_Flag >= 2)
		fSer << t << x_cm << v_cm << u[1][P] << u_t[P] << endl;
